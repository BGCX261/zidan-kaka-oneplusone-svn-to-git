...
class cls_sql_dump
{
    ...
    function get_table_df($table, $add_drop = false)
    {
        ...
        $tmp_arr = $this->db->fetch_first("SHOW CREATE TABLE `$table`");
        $tmp_sql = $tmp_arr['Create Table'];
        $tmp_sql = substr($tmp_sql, 0, strrpos($tmp_sql, ")") + 1); //去除行尾定义。

        if ($this->db->version() >= '4.1')
        {
            $table_df .= $tmp_sql . " ENGINE=MyISAM DEFAULT CHARSET=".($GLOBALS['dbcharset']?$GLOBALS['dbcharset']:'utf8').";\r\n";
        }
        else
        {
    ...
    function get_table_data($table, $pos)
    {
        ...
        /* 获取数据表记录总数 */
        $dtotal = $this->db->fetch_first("SELECT COUNT(*) as total FROM $table");
        ...
        {
            /* 获取数据库数据 */
			$query=$this->db->query("SELECT * FROM $table LIMIT " . ($this->offset * $i + $pos) . ', ' . $this->offset);
            
			while($d=$this->db->fetch_array($query))
			{
    ...
    function dump_table($path, $vol)
    {
        ...
        $this->dump_sql .= '-- END PHPUP SQL Dump Program ';
    ...
    function make_head($vol)
    {
        /* 系统信息 */
        $sys_info['os']         = PHP_OS;
        $sys_info['web_server'] = SITE_ROOT;
        $sys_info['php_ver']    = PHP_VERSION;
        $sys_info['mysql_ver']  = $GLOBALS['db']->version();
        $sys_info['date']       = date('Y-m-d H:i:s');

        $head = "-- PHPUP SQL Dump Program\r\n".
                 "-- " . $sys_info['web_server'] . "\r\n".
                 "-- \r\n".
                 "-- DATE : ".$sys_info["date"]."\r\n".
                 "-- MYSQL SERVER VERSION : ".$sys_info['mysql_ver']."\r\n".
                 "-- PHP VERSION : ".$sys_info['php_ver']."\r\n".
                 "-- PHPUP VERSION : ".VERSION."\r\n".
                 "-- Vol : " . $vol . "\r\n";
    ...
    {
        /* 获取sql文件头部信息 */
        $sql_info = array('date'=>'', 'mysql_ver'=> '', 'php_ver'=>0, 'phpup_ver'=>'', 'vol'=>0);
        $fp = fopen($path,'rb');
        $str = fread($fp, 250);
        fclose($fp);
        $arr = explode("\n", $str);
        ...
        foreach ($arr AS $val)
        {
            ...
            if ($pos > 0)
            {
                ...
                elseif ($type == 'PHPUP VERSION')
                {
                    $sql_info['phpup_ver'] = $value;
                }
                elseif ($type == 'Vol')
                {
                    $sql_info['vol'] = $value;
